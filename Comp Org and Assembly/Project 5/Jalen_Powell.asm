...
; Summary: This program decryptand encrypt a message(input) with a key using Vigenere Cipher, the results will be stored in output.
; Objective: This program has two objectives.The first objective is to create a procedure that will take
; plaintextand an encryption key, use the Vigenère cipher, and encrypt the plaintext.The second
...
MOVEON:
	mov    eax, 0
		...
		Decrypt proc
		mov    esi, 0; index of both inputand output
	...
	mov    eax, 0; clear EAX
		...
		div    d; (5) use divide to get "mod 26".AX will be divided, quotient in ALand reminder in AH..
